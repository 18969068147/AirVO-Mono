...
Initializer::Initializer(FramePtr ReferenceFrame, float sigma, int iterations){
    //从参考帧中获取相机的内参数矩阵K
    // mK = ReferenceFrame->GetCameraInterParam();
    mK =(cv::Mat_<float>(3,3,CV_32F) << 190.978477, 0, 254.931706, 0, 190.973307, 256.897442, 0, 0, 1);
    std::cout << "内参数矩阵K"<< mK << std::endl;
    // 从参考帧中获取去畸变后的特征点
    mvKeys1 = ReferenceFrame->GetAllKeypoints();
    //获取估计误差
    mSigma = sigma;
    mSigma2 = sigma*sigma;
    //最大迭代次数
...
bool Initializer::Initialize(FramePtr CurrentFrame, const std::vector<int> &vMatches12, cv::Mat &R21, cv::Mat &t21,
                             std::vector<cv::Point3f> &vP3D, std::vector<bool> &vbTriangulated){
    // Fill structures with current keypoints and matches with reference frame
    // Reference Frame: 1, Current Frame: 2
    mvKeys2 = CurrentFrame->GetAllKeypoints();
    std::cout << "Initialize: mvKeys2.size =" << mvKeys2.size()<<std::endl;
    mvMatches12.clear();
    ...
    threadH.join();
    threadF.join();
    std::cout << "Initialize:cv::Mat H=" << H <<std::endl;
    std::cout << "Initialize:cv::Mat F" << F <<std::endl;
                      ...
                      cv::Mat &R21, cv::Mat &t21, std::vector<cv::Point3f> &vP3D, std::vector<bool> &vbTriangulated, float minParallax, int minTriangulated)
{
    ...
    std::cout << "ReconstructH SVD::compute V= " << V << std::endl;
    std::cout << "ReconstructH d1/d2= " << d1/d2 <<", "<< "d2/d3="<<d2/d3 << std::endl;
    ...
    std::cout<< "选择最优解。是否满足四个条件 ="<<(secondBestGood<0.75*bestGood && bestParallax>=minParallax && bestGood>minTriangulated && bestGood>0.9*N)<<std::endl;
    std::cout << "secondBestGood="<< secondBestGood <<std::endl;
    std::cout << "bestGood="<< bestGood <<std::endl;
    std::cout << "bestParallax="<< bestParallax <<std::endl;
    std::cout << "minTriangulated="<< minTriangulated <<std::endl;
    std::cout << "minParallax="<< minParallax <<std::endl<<std::endl;

    std::cout << "vR[bestSolutionIdx]="<< vR[bestSolutionIdx] <<std::endl;
    std::cout << "vt[bestSolutionIdx]="<< vt[bestSolutionIdx] <<std::endl;
    ...
    std::cout<< "Initializer::ReconstructH R21="<< R21<<std::endl;
    std::cout<< "Initializer::ReconstructH t21="<< t21<<std::endl;
    // return false;
}
